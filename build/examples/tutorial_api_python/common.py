...
class MPIIPart(Enum):
    ...
    def from_coco(poseKeypoint):
        ...
        for mpi, coco in t:
            if coco.value < 8:
                if poseKeypoint[coco.value][2] == 0.:
                    pose_2d_mpii.append((0, 0))
                    visibilty.append(False)
                    continue
                pose_2d_mpii.append((poseKeypoint[coco.value][0], poseKeypoint[coco.value][1]))
                visibilty.append(True)
            else:
                if poseKeypoint[coco.value + 1][2] == 0.:
                    pose_2d_mpii.append((0, 0))
                    visibilty.append(False)
                    continue
                pose_2d_mpii.append((poseKeypoint[coco.value+1][0], poseKeypoint[coco.value+1][1]))
                visibilty.append(True)
